...
import com.poly.duantotnghiep_jf.Activity.ViewProfileCompany;
...
public class Detail_Company extends AppCompatActivity {
    ...
    private void clickShowProfileCompany() {
        Intent intent = new Intent(Detail_Company.this, ViewProfileCompany.class);
    ...
    private void updateFollowButtonUI(boolean followed) {
        if(followed) {
            btnFollowCompany.setBackground(ContextCompat.getDrawable(Detail_Company.this, R.drawable.bovien_button_viencam));
            btnFollowCompany.setTextColor(ContextCompat.getColor(Detail_Company.this, R.color.orange));
            btnFollowCompany.setText("Unfollow");
        } else {
            btnFollowCompany.setBackground(ContextCompat.getDrawable(Detail_Company.this, R.drawable.bovien_button_cam));
            btnFollowCompany.setTextColor(ContextCompat.getColor(Detail_Company.this, R.color.white));
                ...
                PostHelper.checkIfUserUngTuyenPost(postId, FireBaseHelper.getCurrentUserUid(), new PostHelper.OnCheckUngTuyenListener() {
                    ...
                    public void onPostUngTuyen(boolean isRecruited) {
                        if(isRecruited){
                            ...
                            Toast.makeText(Detail_Company.this, "Bạn đã hủy ứng tuyển cho công việc này.", Toast.LENGTH_SHORT).show();
                            post.setUngTuyenCount(post.getUngTuyenCount() -1);
                            postRef.child("ungTuyenCount").setValue(post.getUngTuyenCount());
                        }
                        else {
                            Toast.makeText(Detail_Company.this, "Ứng tuyển thành công", Toast.LENGTH_SHORT).show();
                        ...
                        new PostHelper.OnCheckLikeListener() {
                            ...
                            public void onPostLiked(boolean isLiked) {
                                if(isLiked){
                                    ...
                                    Toast.makeText(Detail_Company.this, "Đã xóa khỏi danh sách yêu thích", Toast.LENGTH_SHORT).show();
                                    post.setLikeCount(post.getLikeCount() -1);
                                    postRef.child("likeCount").setValue(post.getLikeCount());
                                }
                                else {
                                    likeRef.child(FireBaseHelper.getCurrentUserUid()).setValue(true);
                                    Toast.makeText(Detail_Company.this, "Đã thêm vào danh sách yêu thích", Toast.LENGTH_SHORT).show();
                                    post.setLikeCount(post.getLikeCount() + 1);
                                    postRef.child("likeCount").setValue(post.getLikeCount());
                                }
                            }
    ...
    private void setUpBtnUngTuyen() {
        PostHelper.checkIfUserUngTuyenPost(postId, FireBaseHelper.getCurrentUserUid(), new PostHelper.OnCheckUngTuyenListener() {
            ...
            public void onPostUngTuyen(boolean isRecruited) {
                if(isRecruited){
                    btn_UngTuyen.setBackground(ContextCompat.getDrawable(Detail_Company.this, R.drawable.bovien_button_viencam));
                    btn_UngTuyen.setTextColor(ContextCompat.getColor(Detail_Company.this, R.color.orange));
                    btn_UngTuyen.setText("Hủy ứng tuyển");
                }
                else {
                    btn_UngTuyen.setBackground(ContextCompat.getDrawable(Detail_Company.this, R.drawable.bovien_button_cam));
                    btn_UngTuyen.setTextColor(ContextCompat.getColor(Detail_Company.this, R.color.white));
